...
a.hash)});a.creneau.etranger&&(b.contenu=e.trustAsHtml(b.contenu))})});a.set_creneau_date_due=function(b){var c=_(a.creneaux_devoirs_possibles).findWhere({date_due:b.date_due});b.creneau_emploi_du_temps_id=c.id;a.is_dirty(b)};var D=function(a,b,c){return u.get_creneaux_emploi_du_temps_similaires({id:a.id,debut:k(a.heure_debut.toISOString()).subtract(b,"weeks").toDate(),fin:k(a.heure_debut.toISOString()).add(c,"weeks").toDate()})};D(a.creneau,2,8).then(function(b){a.creneaux_devoirs_possibles_duplication=
[];a.creneaux_similaires=_.chain(b.data).reject(function(a){return _(a.regroupement_id).isUndefined()||a.has_cours}).map(function(b){b.classe=_(a.classes).findWhere({id:parseInt(b.regroupement_id)});b.heure_debut=new Date(b.heure_debut);b.heure_fin=new Date(b.heure_fin);return b}).value();a.creneaux_similaires.selected=[]});D(a.creneau,0,8).then(function(c){_(c.data).isEmpty()&&(c.data=[a.creneau]);a.creneaux_devoirs_possibles=_.chain(c.data).select(function(b){return b.regroupement_id===a.creneau.regroupement_id}).map(function(c){c.classe=
...
!0)};a.creneau_cible_duplication_SP_updated=function(){D(a.creneaux_similaires.selected,0,4).then(function(c){a.creneaux_devoirs_possibles_duplication=_.chain(c.data).select(function(b){return b.regroupement_id==a.creneaux_similaires.selected.regroupement_id}).map(function(c){c.classe=_(a.classes).findWhere({id:parseInt(c.regroupement_id)});c.date_due=b("date")(c.start,"y-MM-dd");c.semaine=k(c.start).from(k(a.creneau.heure_debut),!0)+" plus tard";c.heure_debut=new Date(c.heure_debut);c.heure_fin=
...
b,a.cours.editable=!0):init_cours_existant(C)})}]);
