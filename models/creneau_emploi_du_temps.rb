...
module SemainesDePresenceMixin
  ...
  def pretty_print_semainier
    vacances = CahierDeTextesApp::Utils::Holidays.get( 'A', CahierDeTextesApp::Utils::Holidays.year_rentree ) # FIXME: hardcoded zone A
    semainier = semaines_de_presence.to_s( 2 )
                                    .ljust( 52, '0' )
    semainier = "#{semainier}#{semainier[semainier.length - 1]}"
    semainier[0] = ''
    semainier = semainier.split( '' )
                         .map
    ...
    month_name_length = semainier.keys.map(&:length).max
    puts semainier.keys.map { |month_name| "#{'|' + ' ' * (month_name_length - month_name.length)}#{month_name}" }.join
    puts semainier.map { |month| '|' + "#{semaine_to_s.call( month[1][0] )}#{semaine_to_s.call( month[1][1] )}".ljust( month_name_length, ' ') }.join
    puts semainier.map { |month| '|' + "#{semaine_to_s.call( month[1][2] )}#{semaine_to_s.call( month[1][3] )}".ljust( month_name_length, ' ') }.join
    puts semainier.map { |month| '|' + "#{semaine_to_s.call( month[1][4] )}   ".ljust( month_name_length, ' ') }.join
